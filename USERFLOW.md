...
в целом), потом семь ролевых флоу — что видит и делает **конкретный человек**.
...
Раньше эти четыре роли жили на одной схеме. Разнесли по отдельным: сквозной
путь показывает жизненный цикл выше, а здесь у каждой роли свой микрофлоу, и
их удобно читать и править по одному.

У председателя два шлагбаума: организация утверждается **до** публикации,
результаты принимаются **после** игры. Всё, что между ними, — зона главного
судьи турнира.
...
Зафиксированы в [TZ.md](TZ.md) (§4.4, §8):
...
нижестоящая. Пунктир от администратора — только заведение учётных записей:
судейских прав он не наследует, кто судит турнир решает судейская коллегия.

```mermaid
flowchart TD
    adm["Администратор системы — вся платформа, техническая роль"]
    psk["Председатель судейской коллегии — все турниры"]
...
**Два шлагбаума федерации:** организация утверждается до публикации, результаты
принимаются после игры. Первый решает, состоится ли турнир в таком виде, второй
...
flowchart TD
    ...
    organize["ГС организует турнир: формат, дисциплина, расписание, столы"]
    gateOrg{"ПСК: организация утверждена?"}
    ...
    more{"Остались матчи?"}
    closeT["ГС закрывает турнир — ввод заблокирован"]
    gateRes{"ПСК: результаты приняты?"}
...
**два шлагбаума**: утверждает организацию до публикации и принимает результаты
после игры. Сам турнир не ведёт.
...
Дальше ведёт от организации до сдачи результата — собирает сетку (из библиотеки
или конструктором), задаёт расписание и отправляет организацию на утверждение;
после публикации одобряет заявки, делает посев, назначает судей столов, вызывает
пары и переопределяет спорный счёт, каждая правка идёт в аудит. Закрывает турнир
и сдаёт итоги коллегии, но рейтинг запускает **не он**: пересчёт идёт после
...
flowchart LR
    ...
    organize["Организует: формат, дисциплина, число партий"]
    bracket["Собирает сетку: библиотека или конструктор"]
    schedule["Расписание по дням и столам"]
    submitOrg["Отправляет организацию на утверждение"]
    ...
    closeT(["Закрывает турнир — итоги уходят в коллегию"])
