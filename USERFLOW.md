...
в целом), потом восемь ролевых флоу — что видит и делает **конкретный человек**.
...
Сквозной путь ниже — это **республиканский турнир**, самый длинный из трёх.
Лиги и любительские турниры идут короче: там нет судей на столах, не участвует
судейская коллегия, а значит нет ни конкурса судей, ни двух утверждений
(см. [TZ.md](TZ.md) §4.1). В рейтинг при этом идут все три категории.
...
Одна связная схема на 57 узлов: от заведения турнира до пересчёта рейтинга.
Разбивать её на части нельзя, теряется главное — связь между этапами, поэтому
структура держится на контейнерах, а стрелки идут между ними сквозь всё
полотно.

Кроме основного пути показаны все ветки: развилка по категории турнира,
проверка регистрационного взноса, четыре возврата с причиной (заявка судьи,
система проведения, заявка игрока, итоговый протокол), блокировка старта без
судьи на столе, два режима ввода счёта и обрыв связи.

![Полный ход турнира](diagrams/out/flow-tournament-full.svg)
...
![Тренер, представитель](diagrams/out/flow-coach.svg)
...
flowchart TD
    ...
    direct["ПСК назначает напрямую — малый турнир"]
...
выбирает одного. На малый турнир может назначить напрямую, без конкурса. Дальше
...
flowchart LR
    ...
    pick["Выбирает судью"]
    direct["Назначает напрямую — малый турнир"]
