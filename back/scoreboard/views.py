...
# Пинг решает две задачи. Первая: под WSGI поток занят, пока соединение живо, а
# закрытую вкладку сервер замечает лишь когда пробует в неё написать — с редким
# пингом брошенные соединения держали потоки минутами. Вторая: по нему клиент
# понимает, что соединение живо. Поэтому пинг — именованное событие, а не
# комментарий: комментарии до JS не доходят, и «тихо умерший» поток выглядел бы
# рабочим до конца трансляции.
