...
`wagtail-localize`; пользовательские данные хранятся как введены (QUESTIONS 16.1).
...
**Тестирование — TDD для движка и рейтинга.** Чистая логика с высокой ценой ошибки
пишется **тест-первой**: движок сеток (`bracket.py`), рейтинг судей S1–S4, расчёт
расписания, инварианты (стол не занят дважды, без задвоений). Экраны и вёрстка —
не по строгому TDD. Библиотеки по стекам: `pytest`/`pytest-django` (бэк),
`Vitest` + RTL и `Playwright` (веб), `Jest`/`jest-expo` + `Maestro` (мобилка),
`k6` (нагрузка). Подробно — [TESTING.md](TESTING.md).
...
приложении (React Native) он не работает, а зрелой RN-либы для сеток нет —
рендерим сами. По исследованию: **по умолчанию** — нативные `View`
горизонтально-прокручиваемыми раундами (игроку нужен свой путь, а не всё дерево),
коннекторы — тонким **`react-native-svg`**. Для **полного зумируемого дерева на
32–64+** (зрительский вид) `react-native-svg` при пинч-зуме множества путей
проседает по кадрам — там берём **`react-native-skia`** (canvas/GPU, 60fps; +3–5 МБ,
нужен dev-build, он у нас и так есть). Жесты — `react-native-gesture-handler` +
`reanimated`. **WebView с React Flow не берём** (мост медленный, неродно для живого
экрана — «Native Shell, Web Leaf»). Общая с вебом — только **модель данных сетки**.
...
- **Пользовательские данные** (название турнира, имя клуба, город) — хранятся
  **как введены**, без перевода по полям и без колонок на язык (QUESTIONS 16.1):
