...
турнира из данных самой системы (кто, в какой роли, какого статуса турнир),
S2 из профиля судьи. Здесь платформа и есть первоисточник, никакие «официальные
...
гостю. Апелляция: 10 дней после отказа, комиссия рассматривает 10 рабочих дней.
...
erDiagram
    ...
    JUDGE_DOCUMENT {
        ...
        string status "на проверке | принят | отклонён | оспаривается | отказ"
        int points "начислено при приёме"
        uuid reviewedBy FK "комиссия / ГСК"
        datetime rejectedAt "старт окна апелляции 10 дней"
        string appealReason
    }
    JUDGE_SCORE {
        uuid id PK
        ...
        string source "турнир (авто) | категория из профиля (авто) | документ"
        uuid refId "турнир или JUDGE_DOCUMENT"
...
- **`NEWS_POST` — тип страницы Wagtail, готовый CMS внутри того же Django.** Одна
  ...
  регламент, документы).
- **`JUDGE_DOCUMENT` + `JUDGE_SCORE` — рейтинг судей по Положению** (§7.2). Баллы
  делятся на автоматические (S1 — судейство из турниров, S2 — опорный балл за
  категорию из профиля) и подтверждаемые документами (S3/S4 и смена категории) —
  их судья грузит, комиссия проверяет (`JUDGE_DOCUMENT` со статусами и апелляцией).
  Каждое начисление — строка в `JUDGE_SCORE` (год, категория, источник, баллы),
  это и журнал-аудит, и источник **живого** рейтинга: `R = S1+S2+S3+S4` считается
  выборкой, а не хранимым срезом. `year` нужен потому, что активность сбрасывается
  ежегодно, а категория остаётся.
