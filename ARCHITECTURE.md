...
приложении (React Native) он не работает, поэтому в мобилке сетку рисуем своим
рендером на **`react-native-svg`** (стеклянные узлы через `expo-blur` + коннекторы-
пути + жестовый зум/панорама) либо проще — горизонтально-прокручиваемыми раундами.
**Общая между вебом и мобилкой — только модель данных сетки** (из движка), рендер
платформенный; WebView с React Flow не берём (тяжело и неродно для живого экрана).
