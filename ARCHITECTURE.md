...
- **Источник правды — серверное сообщение банка (`postLink`), а не возврат
  ...
  `reference` и `secret_hash`; проверяем `secret_hash` против того, что клали в
  запрос токена.
...
2. **Счёт.** Бэкенд заводит платёж в своей базе и получает наш собственный
   ...
   `postLink`.
3. **Виджет.** Фронт (сайт или приложение через браузер/веб-вью) подгружает
   скрипт платёжной формы банка и открывает её с полученным токеном и счётом.
...
**Что именно вызывать — берём из действующей документации банка и кабинета
мерчанта, а не из памяти.** Названия методов и хостов ePay менялись между
версиями; в коде они живут одним адаптером (`back/` — сервис взносов), чтобы
смена версии не расползлась по проекту. До получения боевых доступов работаем
на тестовом контуре банка.
