...
| Файлы | S3-совместимое хранилище через boto3 — документы к заявкам, фото | `tt_back`: boto3, `AWS_STORAGE_BUCKET_NAME` |
...
| Релиз | `collectstatic` и `migrate` на старте контейнера — так уже устроен `Procfile` в `tt_back` |
| Откат | предыдущий образ, миграции пишутся совместимыми назад |
| Окружения | стенд и продакшн: разные базы, разные бакеты, всё через переменные среды (`DATABASE_URL`, `AWS_STORAGE_BUCKET_NAME`, `ALLOWED_HOSTS`) |
| Процессы | `gunicorn` под API (синхронные воркеры) и отдельный `uvicorn` под SSE |
| Статика | whitenoise отдаёт из контейнера, файлы пользователей — в S3 |

### Что остаётся нерешённым

- **Где всё это физически размещать.** Единственный по-настоящему открытый
  пункт, и он не технический: Закон РК №94-V определяет, где могут лежать
  персональные данные игроков и приложенные к заявкам документы. Прорабатывается
  вместе с федерацией ([TZ.md](TZ.md) §11, §13.1). Всё остальное в деплое от
  площадки не зависит — контейнер поедет куда угодно.
- **Нужен ли Web Push в браузере** (QUESTIONS 11.1).
