...
- **Проверка допуска через внешние сервисы** (SmartConnect.kz — личность,
  DamuMed — медицинский допуск). Ложится на воркер и очередь: заявка создаётся
  сразу, проверки уходят фоном, результат приезжает игроку по SSE и в очередь
  судьи. Появляются две сущности — ADMISSION_CHECK и CONSENT, — и особая
  категория персональных данных, которой у нас до сих пор не было. План,
  состояния, режимы сбоя и порядок работ —
  [docs/integrations-admission.md](docs/integrations-admission.md); до договора
  и песочницы это план, а не решение.
