...
| Push | **FCM** через `firebase-admin`; на устройстве `expo-notifications` | решение этого документа, см. «Уведомления» |
...
**Отправщик — Firebase Cloud Messaging, через `firebase-admin` в Django.**
Выбор между ним и Expo Push Service решается открытым вопросом 11.1. Expo Push
проще: мы уже на Expo, токен один, отправка — один POST на `exp.host`, ключ APNs
заводится через EAS. Но он умеет только приложения. Если 11.1 закроется как
«веб-push нужен», к нему придётся пристроить второй канал на VAPID — два разных
пути отправки для одних и тех же семи событий. FCM закрывает все три поверхности
сразу: Android напрямую, iOS через APNs, браузер тем же SDK. Пока вопрос открыт,
дешевле взять того, кто не заставит переделывать.

Второй довод меньше, но в нашу сторону: «вас вызвали на стол» — уведомление, у
которого есть крайний срок, и лишний посредник в цепочке доставки тут не нужен.

Цена решения: `firebase-admin` в зависимостях и ключ APNs, который заводим сами,
один раз. В приложении это значит `getDevicePushTokenAsync` вместо
`getExpoPushTokenAsync` — нужен родной токен FCM, токен Expo понимает только
релей Expo.
...
| Куда | Чем | Состояние |
|---|---|---|
| Приложение игрока (Android) | FCM напрямую | решено |
| Приложение игрока (iOS) | FCM → APNs | решено |
| Браузер: судьи, тренеры, коллегия, бухгалтер | тот же FCM (Web Push под капотом) | **нужен ли — QUESTIONS 11.1** |
