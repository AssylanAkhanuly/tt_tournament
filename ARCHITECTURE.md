...
предлагаемое решение. Они всплыли на ревизии схемы: это места, где на диаграмме
стрелка есть, а механизма под ней нет.
...
- **Конвейер видео на YouTube.** Наложение счёта на картинку со столов без
  ...
  режимом сбоев.
- **Как событие попадает из API в SSE-процесс.** На схеме «сервис счёта
  публикует событие», но счёт принимает gunicorn, а поток держит uvicorn — это
  два разных процесса, и общей памяти у них нет. Стрелка нарисована, транспорта
  под ней нет. Предлагаемое решение: `LISTEN/NOTIFY` в самом PostgreSQL — база
  уже есть, отдельный брокер (Redis) ради одного канала не заводим. Предел у
  `NOTIFY` — 8000 байт на сообщение, поэтому в событии передаётся только id
  матча, а счёт SSE-процесс дочитывает из базы.
- **Где выполняется фоновая работа.** Три места на схеме предполагают работу
  «потом», а исполнителя для «потом» в дизайне нет: квитанции Expo Push
  забираются отдельным запросом через минуты после отправки, сама рассылка
  семи событий не должна жить в цикле HTTP-запроса, пересчёт рейтинга после
  утверждения — долгая транзакция. Синхронный gunicorn умеет отвечать на
  запросы, и только. Предлагаемое решение: третий процесс-воркер на Railway и
  очередь задач в PostgreSQL (например, procrastinate — она на `LISTEN/NOTIFY`
  и не тянет Redis). Celery не берём по той же причине, что и всюду: не
  добавлять хранилище, пока единственное, что от него нужно, умеет Postgres.
- **В доменной модели нет таблиц под уведомления.** Лента в интерфейсе
  («пропущенный push не теряет действие») — это хранимые строки, а таблицы
  NOTIFICATION в модели нет. Токены устройств («регистрируется при входе,
  отзывается при выходе») — таблица PUSH_TOKEN, её тоже нет. Блоки на схеме
  есть, хранилища под ними нет. Добавить обе при следующей правке domain.d2.
