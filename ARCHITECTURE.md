...
Эти четыре пункта в стеке намеренно пустые. Каждый — отдельное решение, и
записывать их «по умолчанию» значит принять их молча.

- **Транспорт счёта в реальном времени.** Частый опрос против постоянного
  соединения. Влияет на нагрузку при нескольких столах сразу и на поведение при
  плохой сети в зале.
- **Доставка push.** Приложение есть только у игрока; судьи, тренеры и коллегия
  работают с адаптивного сайта, а push в браузер это другая технология со своими
  ограничениями (QUESTIONS 11.1). Вызов на стол судье нужен мгновенно — это
  самое узкое место.
- **Площадка размещения.** Здесь решает не удобство, а Закон РК №94-V о
  персональных данных: где физически лежат данные игроков и приложенные к
  заявкам документы. Прорабатывается вместе с федерацией ([TZ.md](TZ.md) §11,
  §13.1), а не выбирается в пользу первого удобного облака.
- **Конвейер видео на YouTube.** Наложение счёта на картинку со столов без
  оператора ([TZ.md](TZ.md) §6.3) — это отдельная интеграция со своим темпом и
  режимом сбоев.
