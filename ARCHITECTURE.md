...
§7). Пересчёт запускается не по завершении матчей, а после того, как председатель
судейской коллегии утвердил результаты турнира ([TZ.md](TZ.md) §4.5), и не
задваивается при повторе. Считается рейтинг игроков и судей.

**Роли:** **Администратор системы** держит платформу (пользователи, организации,
настройки, контент) и в судейство не входит. Судейство разделено на два уровня:
**Председатель судейской коллегии** заводит турнир, назначает на него судью по
заявке, утверждает организацию и принимает результаты; **Главный судья турнира**
руководит одним турниром (собирает сетку, разбирает заявки игроков, вызывает
пары, закрывает и сдаёт результат). Ниже — судья стола, тренер/представитель,
игрок, гость. Всего семь ролей, см. [TZ.md](TZ.md) §2.
