...
**Путь релиза приложения** — четыре шага, и у каждого своя скорость:

1. **Сборка — EAS Build.** Облако Expo собирает бинарники обеих ОС; свой мак
   для iOS-сборки не нужен.
2. **Тестовый круг до магазина.** iOS — TestFlight, Android — внутренний трек
   Play Console. Судьи и федерация гоняют боевую сборку до публикации, и
   обновления тестерам уходят без ревью, за минуты. Это же ответ на «как
   показать заказчику до релиза» — ссылка-приглашение, не файл по почте.
3. **Релиз в магазины.** Ревью: Apple — обычно день-два, Google — часы. В Play
   есть поэтапная раскатка по проценту пользователей. Важное ограничение:
   **откатить опубликованный бинарник нельзя** — при проблеме только новая
   версия через новое ревью.
4. **Правки по воздуху — `expo-updates`.** JavaScript и ассеты уезжают мимо
   ревью за минуты — этим закрывается срочный фикс, который иначе ждал бы
   Apple сутки. Граница жёсткая: новая нативная библиотека по воздуху не
   уедет, это снова полный круг через магазин.
