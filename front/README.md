...
| `GET /api/scoreboard/<ключ>/` | текущий счёт; при неизменной версии — 304 без тела |
...
### Почему опрос, а не SSE

Оверлей спрашивает счёт несколько раз в секунду. Это дёшево: пока `rev` не
сдвинулся, Django отвечает **304 без тела** — браузер сам шлёт `If-None-Match`,
потому что запрос идёт с `cache: 'no-cache'`.

Постоянное соединение (SSE) на текущем деплое не годится: бэкенд запущен как
`gunicorn ... --workers 2 --threads 4`, то есть каждое висящее соединение
занимает один из восьми потоков на всю платформу, а два процесса не делят
память и не узнают о записях друг друга.

Когда бэкенд переедет на ASGI ради живых уведомлений, рядом встанет
`GET /api/scoreboard/<ключ>/stream/`, а на фронте поменяется один файл —
`src/features/scoreboard-live/useScoreboardChannel.ts`. Ручки, модель, пульт и
плашка останутся как есть.
