...
На пульте только то, чем ведут счёт. Параметры матча — тип (`MT`), круг
(`R 16`), до скольких партий, язык подписи, командная строка и коды стран для
неё — лежат в состоянии доски и правятся через `PUT`. Оператору посреди партии
они не нужны, а место на планшете дороже.
