...
живёт в Django (`back/scoreboard`, строка на стол). Пульт пишет `PUT` с версией `rev`,
обе стороны читают через одно соединение SSE (`…/stream/`) — опроса у клиента
нет. Транспорт заперт в `src/features/scoreboard-live/useScoreboardChannel.ts`,
там же запасной путь на опрос, если поток где-то буферизуется. Подробности,
цена в потоках gunicorn и настройка источника в OBS — в `README.md`.
