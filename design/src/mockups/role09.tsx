...
export function Tours9_1() {
  return (
    <RoleTablet title="Судья · Оралбай Ержан" sub="Мои назначения и открытые приёмы заявок" badge="СЕЗОН 2026">
      ...
      <Hint>
        Приём заявок на судейство показываем, пока не решён вопрос 12.6: сохраняется ли конкурс
        заявок или судью назначают напрямую. Решение уходит председателю ГСК и приходит уведомлением.
      </Hint>
    </RoleTablet>
  );
}
...
export function Table9_2() {
  return (
    <RoleTablet title="Чемпионат Казахстана 2026" sub="Мой стол 4 · день 2 · 13 марта" badge="ИДЁТ">
      ...
      <Hint>
        Матча нет — стол свободен, видна только очередь. Неявка уходит главному судье на решение о
        технической победе; неявка самого судьи при поданной заявке — минус 1 балл S1 (§7.2).
      </Hint>
    </RoleTablet>
  );
}
...
export function Score9_3() {
  return (
    <RoleTablet title="Стол 4 · ввод счёта" sub="Смагулов Алан — Токаев Марат · 1/8 финала · партия 4" badge="ИДЁТ">
      ...
      <Hint>
        Обрыв связи: ввод продолжается на планшете, вместо «связь есть» — «N событий в очереди»;
        после восстановления события уходят по порядку. При расхождении приоритет у судьи стола — он
        видит игру (§12). После подтверждения счёт правит только главный судья.
      </Hint>
    </RoleTablet>
  );
}
...
export function Log9_4() {
  return (
    <RoleTablet title="Стол 4 · история матча" sub="Смагулов Алан — Токаев Марат · 1/8 финала" badge="ИДЁТ">
      ...
      <Hint>
        Только просмотр: исправить событие задним числом судья стола не может — правка идёт через
        главного судью. В режиме «по партиям» в ленте остаются партии и служебные события, розыгрышей
        по одному очку нет.
      </Hint>
    </RoleTablet>
  );
...
export function Rating9_5() {
  return (
    <RoleTablet title="Мой рейтинг судьи" sub="Сезон 2026 · R = S1 + S2 + S3 + S4 (§7.2)" badge="ОПУБЛИКОВАН">
      ...
      <Hint>
        Зачёт есть: баллы в 4 категориях из 4, S1 и S2 на месте (12 место из 168 судей). С нового
        года S1, S3 и S4 обнуляются, балл за категорию S2 остаётся (§7.2).
      </Hint>
