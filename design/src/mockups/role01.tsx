...
/** Рабочая очередь: каждый счётчик открывает список с готовым фильтром. */
export const ATTENTION = [
  { n: '3', t: 'без главного судьи', to: 'Э1.2' },
  { n: '5', t: 'регламент не заполнен', to: 'Э1.2' },
  { n: '12', t: 'заявки без решения > 3 дней', to: 'Э1.3' },
  { n: '18', t: 'взносы просрочены' },
];

/** Строка очереди под плитками: коротко и в один ряд, без крупных плиток. */
export const Attention = ({ act = true }: { act?: boolean }) => (
  <div className="mkattn">
    <span className="mkattn-h">Требует внимания</span>
    {ATTENTION.map((a) => (
      <span className="mkattn-i" key={a.t} data-to={act ? a.to : undefined}>
        <b>{a.n}</b> {a.t}
      </span>
    ))}
  </div>
...
export function Cal1_2({ variant }: { variant?: DeskVariant }) {
  return (
    <RoleScreen
      ...
      sub="Сезон 2026 · 32 соревнования"
    >
      <div className="dactionbar">
        <div className="dseg2">
...
export function Reg1_6() {
  return (
    <RoleScreen
      ...
      nav="Реестры"
      title="Реестры"
      sub="Спортсмены, судьи, тренеры, клубы"
    >
      <div className="ttabs">
