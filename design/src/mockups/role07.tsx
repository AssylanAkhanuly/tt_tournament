...
export function Desk7_1() {
  return (
    <RoleScreen
      ...
      hint="Заявки рассматривает главный судья — экрана заявок у секретаря нет. Секретарь оформляет по решению судьи."
    >
      <Chips
        items={[
      ...
      <div className="mkcols">
        ...
        <Panel title="Решение главного судьи · чтение" extra={<P t="ТОЛЬКО ПРОСМОТР" cls="wait" />}>
          ...
          <div style={{ height: 12 }} />
          <Hint>
            Формат выбирает главный судья (Э6.3), секретарь собирает по нему. Решения по заявкам
            игроков секретарь не принимает и их экрана не видит.
          </Hint>
        </Panel>
      </div>
    </RoleScreen>
...
export function Draw7_2() {
  return (
    <RoleScreen
      ...
      hint="Перебросить жребий можно, пока сетку не утвердил главный судья: прежний результат остаётся в журнале."
    >
      <Chips
        items={[
          { v: '128', k: 'Участников', tone: 'b' },
      ...
      <div className="mkcols">
        ...
        <Panel title="Способ распределения" extra={<P t="ЧЕРНОВИК ЖРЕБИЯ" cls="wait" />}>
          ...
          <div style={{ height: 12 }} />
          <Hint>Кто отвечает, если главный судья не согласен с результатом жеребьёвки, — открытый вопрос к федерации.</Hint>
        </Panel>
      </div>
    </RoleScreen>
  );
...
export function Bracket7_3() {
  return (
    <RoleScreen
      ...
      hint="Секретарь делает — судья утверждает. Публиковать сетку без утверждения главного судьи секретарь не может."
    >
      <Chips
        items={[
          { v: '128', k: 'Участников', tone: 'b' },
      ...
      <div className="mkcols">
        ...
        <Panel title="Вводные от судьи" extra={<P t="СОБРАНА 15.05" cls="live" />}>
          ...
          <div style={{ height: 12 }} />
          <Hint>Формат менять нельзя: его выбрал главный судья. Секретарь собирает сетку по этому решению.</Hint>
        </Panel>
      </div>
    </RoleScreen>
  );
...
export function Schedule7_4() {
  return (
    <RoleScreen
      ...
      hint="Секретарь расставляет матчи, главный судья проверяет. Конфликт — когда игрок стоит на двух столах в одно время."
    >
      <Chips
        items={[
          { v: '3', k: 'Игровых дня' },
...
export function Protocols7_5() {
  return (
    <RoleScreen
      ...
      hint="Секретарь заполняет протокол и передаёт главному судье: на утверждение коллегии отправляет судья (Э6.7)."
    >
      <Chips
        items={[
      ...
      <div className="mkcols">
        ...
        <Panel title="Оформление" extra={<P t="В РАБОТЕ" cls="wait" />}>
          ...
          <div style={{ height: 12 }} />
          <Hint>
            Если коллегия вернёт протокол, секретарь увидит причину и поправит оформление.
            Результаты матчей исправляет только главный судья.
          </Hint>
        </Panel>
      </div>
    </RoleScreen>
