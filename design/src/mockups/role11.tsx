...
export function Cands11_1() {
  return (
    <RoleScreen
      ...
      hint="Роль наблюдательная: данные не меняются, звёздочка — личная пометка тренера."
    >
      <Chips
        items={[
      ...
      <Hint>
        ⚠ 12.1: видит ли роль всех спортсменов страны или только кандидатский список — не решено;
        пока показываем весь реестр как чтение. Заявка сборной на международные старты в календаре
        §4.1 не предусмотрена — этого экрана в макете нет.
      </Hint>
    </RoleScreen>
  );
}
...
export function Card11_2() {
  return (
    <RoleScreen
      ...
      hint="Экран целиком только для просмотра: тренер сборной ничего не редактирует."
    >
...
export function Compare11_3() {
  return (
    <RoleScreen
      ...
      hint="Сравнение собирается из реестра: рейтинг, результаты стартов, личные встречи."
    >
      <Panel
      ...
      >
        ...
        <Hint>
          Гипотеза до ответа федерации (12.1): роль только смотрит. Если появится отбор «список
          сборной» и заявка на международные старты, экран станет местом, где состав собирают, — и
          макет пересобирается.
        </Hint>
      </Panel>
    </RoleScreen>
  );
