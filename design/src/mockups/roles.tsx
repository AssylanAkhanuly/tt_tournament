...
export const R05: RoleUI = {
  ...
  nav: [
    /* «Панель» и «Соревнования» — разные вещи ✳: панель отвечает «что решить
       ...
       («Мои соревнования»), панель работала за оба, и попасть на конкретный
       турнир было неоткуда: очередь уводила сразу в дело. */
    [<LayoutDashboard size={18} />, 'Панель'],
    [<Trophy size={18} />, 'Соревнования'],
    /* «Судьи» — список судей, а не судьи по турнирам ✳: наряд собирается в
       карточке самого турнира, а этот пункт отвечает на вопрос «кто у нас
       вообще есть» — реестр с допуском и рейтингом (Э5.5). Пунктов было два,
       и оба вели в списки людей: «Судьи» (по турниру) и «Рейтинг судей». */
    [<Gavel size={18} />, 'Судьи'],
...
export const RJUDGE: RoleUI = {
  ...
  nav: [
    ...
    [<ListChecks size={18} />, 'Рейтинг судей'],
  ],
};

/* Роль 15 — председатель судейской коллегии региона ✳ (замечание федерации,
   09.2026). Первая роль с географической областью: срез — регион, а не турнир
   и не «своё». В шапке поэтому стоит регион, а не сезон: председатель всё
   время работает в одном и том же участке, и напоминать ему, в каком именно,
   надо не подписью на экране, а самим брендом оболочки. */
export const R15: RoleUI = {
  num: '15', title: 'Председатель СК региона',
  person: { nm: 'Сейтқали А.', rl: 'Председатель СК · Павлодар', av: A(64) },
  brandName: 'Судейская коллегия · Павлодар', brandSub: 'Судьи региона · наряды · начисления',
  badge: false,
  nav: [
    [<Gavel size={18} />, 'Судьи региона'],
    [<ClipboardList size={18} />, 'Наряды региона'],
    [<Scale size={18} />, 'Начисления'],
  ],
};

/* Дисциплинарный комитет — орган, а не роль ✳: человек входит в него поверх
   своей роли и получает эти экраны, пока состоит в составе. Оболочка своя,
   потому что комитет живёт не турниром, а делами: в шапке не старт, а сезон. */
export const RDISC: RoleUI = {
  num: '16', title: 'Дисциплинарный комитет',
  person: { nm: 'Мукашев Б.', rl: 'Член комитета · председатель ГСК', av: A(67) },
  brandName: 'Дисциплинарный комитет', brandSub: 'Протесты · нарушения · решения',
  badge: false,
  nav: [
    [<Scroll size={18} />, 'Протесты'],
...
export const R10: RoleUI = {
  ...
  /* Разделы открытого турнира ✳ (28.08.2026). Пункт был один — «На контроле», —
     и это ломало флоу: инспектор заходил в турнир и оказывался в тупике, из
     которого не видно ни где он сейчас, ни куда идти дальше. Сайдбар с одним
     пунктом подсвечивал «На контроле» на всех четырёх экранах сразу.

     Прежнее решение (19.08.2026) боялось другого: что «ход», «правки» и
     «заключение» станут списками по всем турнирам сразу — инспектор так не
     работает. Это остаётся верным, поэтому пункты — не разделы системы, а
     **разделы того старта, который открыт**: в шапке экрана всегда написано,
     какого именно, а «На контроле» стоит первым и возвращает к списку.
     Турнир не выбран — разделы не ведут никуда и говорят об этом. */
  nav: [
    [<Eye size={18} />, 'На контроле'],
    [<Gavel size={18} />, 'Работа судей'],
    [<History size={18} />, 'Журнал правок'],
    [<FileText size={18} />, 'Заключение'],
  ],
};
...
export const R11: RoleUI = {
  ...
  /* Три раздела ✳ (04.09.2026): кандидаты, карточка на чтение и сравнение
     убраны вместе с экранами — их федерация не просила (вопрос 12.1). */
  nav: [
...
export const R14: RoleUI = {
  ...
  nav: [
    ...
    [<CalendarDays size={18} />, 'Календарь'],
    /* Пункта «Моя заявка» больше нет ✳: заявка относится к одному турниру и
       открывается из его строки в календаре — отдельным разделом она жила
       рядом с турниром, к которому относится. «Мой турнир» вместо «Мой матч»:
       матч — часть турнира, а не наоборот, и на этом экране ещё состав,
       группы и сетка. */
    [<Timer size={18} />, 'Мой турнир'],
