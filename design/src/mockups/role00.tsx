...
const Auth = ({ wide, judge, children }: { wide?: boolean; judge?: boolean; children: ReactNode }) => (
  <DeskFrame>
    <div className="mkauth">
      <div className={'mkauth-card' + (judge ? ' judge' : '')} style={wide ? { width: 540 } : undefined}>
        {children}
      </div>
    </div>
  </DeskFrame>
...
const Login0_1States = () => (
  <States>
    ...
    <Shot tone="danger" title="Неверный логин или пароль" text="Ошибка под полем; поля не очищаются, счётчика попыток нет ✳.">
      <Input label="Телефон или почта" value="+7 705 431 20 18" icon={<User size={15} />} />
      <Input
        label="Пароль"
        value="••••••"
    ...
    >
      <Rows>
        <Row nm="Вход не закрыт" sub="следующая попытка через несколько секунд · разблокировка не требуется" pill={{ t: 'ЖДЁМ', cls: 'wait' }} />
        <Row nm="Владельцу ушло письмо" sub="кто-то подбирает пароль — можно сменить его или включить вход по ссылке" pill={{ t: 'УВЕДОМЛЕН', cls: 'reg' }} />
      </Rows>
      <Alert tone="warning">
        ⚠ Числа — с какой попытки растёт задержка и до скольких секунд — наш проект (TZ §12): сама
        схема «без ручной разблокировки» задана федерацией.
      </Alert>
    </Shot>
...
export function SignUpJudge0_7() {
  return (
    <Auth wide judge>
      <Brand size="lg" />
      <div className="t">Регистрация судьи</div>

      <div className="dform">
        <Input label="Фамилия" placeholder="Оралбай" />
        <Input label="Имя, отчество" placeholder="Ержан Маратович" />
        <Input label="Дата рождения" placeholder="дд.мм.гггг" format="date" />
        <Input label="Телефон" placeholder="+7 ___ ___ __ __" />
        <Input label="Почта" placeholder="имя@домен" />
        <Input label="Пароль" placeholder="не короче 8 знаков" className="dfield wide" />
  ...
  <States>
    ...
    >
      ...
      <Alert>
        ...
        регистрации им не нагружаем: удостоверения может не быть под рукой, а вход в систему из-за
        этого откладывать незачем. Категорию с его слов при этом не ставят: от неё зависят баллы и
        назначения.
      </Alert>
    </Shot>
