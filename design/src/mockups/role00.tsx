...
export function Profile0_2() {
  return (
    <RoleScreen
      ...
      nav="Профиль"
      title="Мой профиль"
      sub="Контакты, язык, пароль и роль"
    >
      <div className="mkcols">
        ...
        {/* Списка своих ролей здесь нет: он был на чтение и ничего не решал, а
            роль человек переключает там, где она написана, — в меню профиля в
            шапке. Кто её выдал и до какого срока — в карточке пользователя у
        ...
        <div style={{ display: 'grid', gap: 12 }}>
          ...
          <Panel title="Роль, под которой я работаю">
            <Row
              nm="Администратор Федерации"
              sub="система · бессрочно"
              pill={{ t: 'СЕЙЧАС', cls: 'live' }}
            />
            <Alert>
              Ролей у человека бывает несколько. Переключается роль в меню профиля в шапке — там же,
              где написано, кто ты сейчас; интерфейс меняется под выбранную.
            </Alert>
          </Panel>
        </div>
      </div>
...
const Profile0_2States = () => (
  <States>
    ...
    <Shot tone="info" title="Роль истекла ✳" text="Из переключателя пропадает, доступ по ней закрыт.">
      <Rows>
