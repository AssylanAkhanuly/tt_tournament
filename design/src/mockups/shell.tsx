...
export type RoleUI = {
  ...
  /** Роли этого человека, если их несколько: переключаются в меню профиля.
      Строкой — название роли; парой `{ t, to }` — ещё и экран, на который она
      открывается (тип совпадает с `RoleUI` нового слоя). */
...
export type ScreenEntry = {
  ...
  /** Тот же экран во втором формате ✳ (30.08.2026): у десктопной роли — на
      телефоне, у телефонной — на десктопе. Полный адаптив решён по всем
      экранам: роль работает не только с того устройства, под которое её
      рисовали первым. Кадр показывается врезкой под основным макетом. */
  alt?: () => ReactNode;
  /** Другие кадры того же экрана ✳ (31.08.2026): следующий шаг, другое
      положение, тот же шаг на телефоне. Не состояние (`States` — «тот же экран
      в другой ситуации») и не второй формат, а другой кадр одного экрана: у
      аттестации это «до теста», «тест идёт» и «итог».

      Заведены полем, а не разметкой внутри `view`, потому что кадры нужны двум
      видам сразу и по-разному: борд ставит их врезками одну под другой, а
      карта — переключателем в шапке панели. Пока кадры лежали внутри `view`,
      карта показывала их простынёй вниз, и экзамен на ней находился только
      прокруткой. */
  frames?: { t: string; view: () => ReactNode }[];
};
        ...
        {items.map(([code, s], i) => (
          <Fragment key={code}>
            <Screen code={code} cap={s.cap}>
              {s.view()}
              {/* Второй формат того же экрана — врезкой под основным: полный
                  адаптив решён по всем экранам (30.08.2026), и на борде он
                  должен читаться без кликов. */}
              {s.alt && <Also cap="Тот же экран во втором формате">{s.alt()}</Also>}
              {/* Другие кадры того же экрана — такими же врезками: на борде
                  они читаются подряд, а карта покажет их переключателем. */}
              {s.frames?.map((f) => (
...
export const ST = {
  ...
  /** Мест не осталось, заявка ждёт своей очереди ✳ (замечание федерации,
      09.2026): судья заявляется один раз, лишние заявки заменяет резерв. */
  reserve: { t: 'В РЕЗЕРВЕ', cls: 'reg' as const },
