...
/** контраст по WCAG: 1 — не различить, 21 — чёрное на белом */
const contrast = (a: string, b: string) => {
  const [hi, lo] = [luma(a), luma(b)].sort((x, y) => y - x);
  return (hi + 0.05) / (lo + 0.05);
};
/* Текст поверх заливки: берём тот из двух, что даёт БОЛЬШЕ контраста, а не тот,
   что подсказывает порог яркости.

   Раньше стояло `luma > 0.45 ? тёмный : белый`, и порог врал: у 16 тем из 32
   белая подпись на акценте не дотягивала до 4.5 : 1 (норма AA для подписи 16 px
   полужирным). Хуже всех было там, где акцент яркий: «Степь» 2.11, «Океан»
...
export const THEMES: Theme[] = [
  ...
  { id: 'daylight', label: 'Светлая', group: 'Светлые', note: 'Дневная тема на нейтральном интерфейсном синем.',
    /* Было #2f6bff — ровно на волосок ниже нормы: белая подпись давала 4.499
       при требуемых 4.5. Синий сдвинут на пару ступеней темнее. */
    seeds: light({ accent: '#2a64f5' }) },
  ...
  /* Ещё три светлые — добавлены 22.08.2026, когда выбирали цветовую гамму
     мобильного экрана: у роли всё рисуется на светлой, а светлых было пять. */
  { id: 'daylight-tengri', label: 'Светлая · тенгри', group: 'Светлые', note: 'Голубой флага вместо синего знака: та же семья, но звонче.',
    seeds: light({ accent: '#00a3c4' }) },
  { id: 'daylight-indigo', label: 'Светлая · индиго', group: 'Светлые', note: 'Сине-фиолетовый: спокойнее флагового и заметно строже.',
    seeds: light({ accent: '#4f46e5' }) },
  { id: 'daylight-cherry', label: 'Светлая · вишня', group: 'Светлые', note: 'Тёмно-красный акцент — язык спортивных клубов, а не сервисов.',
    seeds: light({ accent: '#be123c' }) },
];
