...
const ACCESS: Row[] = [
  ...
  /* Строка «Пароль · изменён 02.02.2026» стояла здесь до 30.08.2026 ✳: пароля
     в системе больше нет — вход по ИИН и одноразовому коду через Smart Bridge
     (Э0.1). Осталось то, ради чего пароль и меняли, — закрыть чужой сеанс. */
  {
    ic: <KeyRound size={17} />,
    nm: 'Вход · ИИН •••• •••• 0123',
    ss: 'код приходит SMS на +7 7•• ••• 44 03 · пароля нет',
    val: 'Выйти везде',
  },
  {
    ic: <Settings size={17} />,
...
const SET_LINKS: { nm: string; ic?: ReactNode; off?: boolean; to?: string }[] = [
  { nm: 'Выйти на всех устройствах', ic: <KeyRound size={15} /> },
