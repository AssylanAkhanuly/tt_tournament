/* Э14.13 «Новости» и Э14.14 «Новость» в языке присланного референса
   ...
   две вещи, которых в первой версии не было:
     ...
     — **фильтры**: лента чипов по темам. Это отменяет прежнее решение по
       Э14.13 «фильтра по темам на экране нет» (спортсмен читает ленту сверху
       вниз). Отмена записана в `flows/14-sportsmen.md`. Второй ряд фильтра —
       круглые обложки турниров — был и снят 29.08.2026: у федерации нет
       снимков турниров, и кружки повторяли те же пять фотографий, что и
       карточки ленты;
   ...
   Цвета — системные. Палитру референса (почти чёрный чип, оранжевая пилюля,
   свой синий) попробовали и сняли 29.08.2026: три чужих акцента спорили с
   синим и жёлтым знака ФНТ, а экран выпадал из роли. Токены `--n-*` остались
   слоем имён и ссылаются на системные `--c-*`, поэтому новости снова следуют
   теме вместе с профилем и аналитикой.
