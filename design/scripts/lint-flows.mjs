...
/* Сквозные экраны Э0.x описаны один раз и разрешены и в переходах, и в макетах
   ...
   Их два набора: `role00.ts` — вход, профиль, уведомления, публичная часть и
   регистрации; `role00j.ts` — кабинет судьи, общая передняя судейских ролей
   (заявки на судейство и рейтинг). Роль судьи ссылается на кабинет так же, как
   любая роль ссылается на вход. */
const COMMON = new Set(
  ['role00.ts', 'role00j.ts'].flatMap((f) =>
    all(TS_SCREEN, readFileSync(join(DATA_DIR, f), 'utf8')).map((m) => m[1]),
  ),
);
